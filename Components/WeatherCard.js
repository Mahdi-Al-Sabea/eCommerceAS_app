...
function WeatherCard () {
  ...
  return (
    <Card style={styles(colors).card} elevation={3}>
      <Card.Title
      ...
      <Card.Content>
        <View style={styles(colors).row}>
          ...
          <Text style={styles(colors).value}>{temperature_2m}°C</Text>
        </View>

        <View style={styles(colors).row}>
          <MaterialCommunityIcons name="water-percent" size={22} color="#039BE5" />
          <Text style={styles(colors).value}>{relative_humidity_2m}% Humidity</Text>
        </View>

        <View style={styles(colors).row}>
          <MaterialCommunityIcons name="weather-windy" size={22} color="#009688" />
          <Text style={styles(colors).value}>{wind_speed_10m} km/h Wind</Text>
        </View>
      </Card.Content>
    </Card>
  );
